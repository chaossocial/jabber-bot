...
def handle_notification(notification):
    ...
    if status['account']['username'] != status['account']['acct']:
        mastodon.status_reply(to_status=status['id'], status='Sorry, our Jabber accounts are only available to inhabitants of chaos.social.')
        return
    user = status['account']['username']
    if has_jabber_account(user):
        mastodon.status_reply(to_status=status['id'], status='{}@jabber.chaos.social is already your registered jabber account – you can register only once.'.format(user))
        return
    try:
        password = create_jabber_account(user)
        mastodon.status_reply(to_status=status['id'], status='Your Jabber account has been created:\n\nUser: {user}@jabber.chaos.social\nPassword: {password}\n\nPlease change the password soon, using a client like Pidgin.'.format(user=user, password=password), visibility='private')
    except Exception as e:
        if isinstance(e, subprocess.CalledProcessError) and 'That user already exists' in e.output:
            mastodon.status_reply(to_status=status['id'], status='{}@jabber.chaos.social is already your registered jabber account – you can register only once.'.format(user))
        else:
            print(e)
            mastodon.status_reply(to_status=status['id'], status='Oops, I could not register your account. Please stay tuned, I will page my admins.', visibility='private')
